...
public sealed partial class DuplicateExtensionsViewModel : ObservableObject
{
    ...
    private async Task RefreshAsync()
    {
        ...
        try
        {
            _rows = await Task.Run(() => _audit.FindDuplicateExtensionRecords(_store.Context));
            ApplyFilter();
            SummaryText = $"Rows: {Rows.Count}; DuplicateNumbers: {Rows.Select(r => r.ExtensionNumber).Distinct(StringComparer.OrdinalIgnoreCase).Count()}";
        }
    ...
    private void ApplyFilter()
    {
        ...
        var filtered = _rows.Where(r =>
                Contains(r.ExtensionNumber, q)
                || Contains(r.ExtensionId, q)
                || Contains(r.OwnerType, q)
                || Contains(r.OwnerId, q)
                || Contains(r.ExtensionPoolId, q))
            .ToList();
